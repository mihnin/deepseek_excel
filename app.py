...
# Функция для отображения настроек LLM (локальных и облачных)
def llm_settings_ui():
    """Отображает настройки LLM с поддержкой как облачных, так и локальных моделей"""
...
# Функция для получения экземпляра LLM провайдера
def get_llm_provider(settings):
    """
    Возвращает настроенный экземпляр LLM провайдера на основе текущих настроек.
    
    Args:
        settings: Настройки из llm_settings_ui()
        
    Returns:
        LLM провайдер (облачный или локальный)
    """
    if settings["provider_type"] == "cloud":
        ...
        try:
            ...
            st.error("Не удалось импортировать LLMServiceProvider. Проверьте наличие модуля llm_integration.py")
            return None
    else:
        # Импортируем класс для локальных LLM
        try:
            from local_llm_integration import LocalLLMProvider
            ...
            st.error("Не удалось импортировать LocalLLMProvider. Проверьте наличие модуля local_llm_integration.py")
            return None

# Функция для создания универсального LLM провайдера
def get_unified_llm_provider(settings):
    """
    Возвращает универсальный LLM провайдер.
    ...
    """
    try:
        from local_llm_integration import UnifiedLLMProvider
        ...
        st.error("Не удалось импортировать UnifiedLLMProvider. Проверьте наличие модуля local_llm_integration.py")
        return None
